...
public class ExceptionService {
	...
	public void ex1() {
		...
		// catch(예외){} : try 구문에서 발생한 예외를 잡아내서 처리하고, 프로그램이 비정상 종료되지 않도록 함
		
		
		try {
			System.out.println("결과: " + input1 / input2);
			//ArithmeticException: 산술적 에외
			...
			System.out.println("infinity : 0으로 나눌 수 없습니다.");//처리 코드
	...
	public void ex3() {
		...
		try {
			...
			//	다형성 업캐스팅에 의해서 모두 잡아 처리됨! **
			
			//catch문 연속 작성 시 Exception 상속 구조들 주의하자!(다형성)
			//최상위 부모인 Exception은 맨 아래에 작성해야 함!
			//--> 자식 예외들 에러 뜸 unreached
	...
	public void ex4() {
		...
		try {
			System.out.println(4 / 0); //ArithmethicException 발생
		} catch (ArithmeticException e) { 
			...
			System.out.println( e ); // e.toString(); 과 같음
	...
	public void methodD() {
		...
		//	- 프로그램 수행 중 개발자의 코딩 실수 또는 사용자의 잘못된 값 입력으로
		// 	  흔하게 발생할 수 있는 예외
		// 		--> if문 같은 단순 코드로 해결 가능
