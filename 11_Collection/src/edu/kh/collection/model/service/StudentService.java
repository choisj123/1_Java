...
public class StudentService {
	...
	//		--> 너무 큰 값을 작성하면 메모리를 많이 소모함, 리스트 크기 지정가능하지만 의미 없음!
	...
	public void ex() {
		...
		//studentList.add(sc); //1 		//제네릭 --> Sudent 객체 아니라서 에러
		//studentList.add("문자문자"); //2
		//studentList.add(new Object()); //3
		//studentList.add(15); //4
		...
		// 다운 캐스팅을 해야 ( get(int index) 메소드가 반황형이 Object이기 때문에
	...
	//alt + commend+ j / alt + shift + j(window)
	/** 메뉴 출력용 메서드
	 * @author 98chltnwls@gmail.com
	 */
	public void displayMenu() {
		...
		do {
			System.out.println("\n==========학생 관리 프로그램===========");
			System.out.println("1. 학생 정보 추가"); //addStudent
			System.out.println("2. 학생 전체 조회"); //selectAll
			System.out.println("3. 학생 정보 수정"); //updateStudent
			System.out.println("4. 학생 정보 제거"); //removeStudent
			System.out.println("5. 이름으로 학생 검색(일치)"); //searchName1
			System.out.println("6. 이름으로 학생 검색(포함)"); //searchName2
	...
	public void selectAll() {
		...
		*/
		...
		for( Student std : studentList ) {
			//std에는 for문 반복 시 마다 0,1,2,... 인덱스 요소들이 한번씩 저장됨
	...
	public String updateStudent() throws InputMismatchException {
		...
		// 1) 학생 정보가 studentList에 있는가? if else
		if( studentList.isEmpty() ) {
			return "입력된 학생 정보가 없습니다.";
	
		// 2) 입력된 숫자가 0보다 작은가? (음수 X) if else
		}else if (index < 0) {
			return "음수는 입력할 수 없습니다.";
			
		// + 문자열을 입력한 경우 --> throws 예외처리
		...
		// 3) 입력된 숫자가 studentList 범위 내 인덱스 번호인가?
			...
			System.out.println(studentList.get(index)); //객체 얻어오기
