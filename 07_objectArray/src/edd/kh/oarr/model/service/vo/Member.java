package edd.kh.oarr.model.service.vo;

public class Member {
	//필드(==멤버 변수)
	private String memberId;
	private String memberPw;
	private String memberName;
	private int memberAge;
	private String region; //지역(서울,경기,충북,전남...)
	
	//생성자
	//생성자 규칙
	// 1. 생성자 이름은 클래스명과 같아야한다.
	// 2. 반환형이 없다.
	
	//기본생성자
	public Member() {}
	
	//매개변수 생성자
	public Member(String memberId, String memberPw, String memberName, int memberAge, String region) {
		
	this.memberId = memberId;
	this.memberPw = memberPw;
	this.memberName = memberName;
	this.memberAge = memberAge;
	this.region = region;
	
	}
	
	
	//메소드
	//getter / setter
	public String getMemberId() {
		return memberId;
	}
	//public 외부클래스에서도 접근가능 (다 접근 가능)
	//String 자료형 반환
	//매개변수가 없어서 중복된 이름의 memberId가 없기때문에 memberId라고만 써도 ok
	//getter 필드에 있는 아이디를 돌려받는 것

	public void setMemberId(String memberId) {
		this.memberId = memberId;
		//return;
		//모든 매서드는 종료시 호출된 곳으로 돌아가는 return 구문이 작성되어야 하지만
		//단, void일 경우 생략 가능하다
		//-->생략시 컴파일러가 자동 추가
	}
	
	// public 외부클래스에서도 접근가능 (다 접근 가능)
	// void 매개변수로 받아온 값을 세팅만 해주고 반환해주지 않음
	// setter는 말 그대로 셋팅하는 애
	// 무엇을 셋팅할지 전달받아올 아이가 필요하다 --> 매개변수
	// 외부에서 매개변수로 받아온 값을 this를 이용하여 필드에 있는 변수에 저장해줌
	// setter는 보통 반환 값은 없음.--> void
	
	public String getMemberPw() {
		return memberPw;
	}

	public void setMemberPw(String memberPw) {
		this.memberPw = memberPw;
	}

	public String getMemberName() {
		return memberName;
	}

	public void setMemberName(String memberName) {
		this.memberName = memberName;
	}

	public int getMemberAge() {
		return memberAge;
	}

	public void setMemberAge(int memberAge) {
		this.memberAge = memberAge;
	}

	public String getRegion() {
		return region;
	}

	public void setRegion(String region) {
		this.region = region;
	}
	
	
	
}
