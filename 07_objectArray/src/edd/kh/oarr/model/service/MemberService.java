...
public class MemberService {
	...
	public MemberService() {
		...
		memberArr[1] = new Member("user02", "pass02", "홍길순", 24, "경기");
		memberArr[2] = new Member("user03", "pass03", "고길동", 17, "전남");
	}
	...
	public String login() {
		...
		for(int i = 0; i < memberArr.length; i++) { //회원 정보가 있을 경우
			//2) 회원 정보(memberArr[i])의 아이디, 비밀번호와 입력받은 아이디 비밀번호가 같은지 확인			
			if(memberArr[i] != null) {
				if(memberArr[i].getMemberId().equals(memberId) && 
				   memberArr[i].getMemberPw().equals(memberPw)) {
					//3) 중첩 if문 - 로그인 회원 정보 객체(Member)를 참조하는 변수 loginMember에
					//	현재 접근 중인 memberArr[i] 요소에 저장된 주소를 얕은 복사(대입)
	...
	public void searchRegion() {
		...
		for (int i = 0; i < memberArr.length; i++) {
			...
			//		입력받은 지역(inputRegion)과 같을 경우, 회원 아이디/이름 출력
